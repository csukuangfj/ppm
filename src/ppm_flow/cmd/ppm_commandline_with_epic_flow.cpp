...
int main(int argc, char* argv[])
{
   std::vector<std::vector<cv::String> > data{
         ...
         {
               cv::String(KFJ_DATA_PATH) + "/middlebury_flow/Venus/frame10.png",
               cv::String(KFJ_DATA_PATH) + "/middlebury_flow/Venus/frame11.png",
               cv::String(KFJ_DATA_PATH) + "/middlebury_flow/Venus/flow10.flo",
         }, // 3
   };
   ...
   if (index == 0)
   {
      config.set_grid_space(4);
      config.set_pyramid_ratio(0.75);
      config.set_number_of_pyramid_levels(4);
   }
   else if ((index == 1) || (index == 2) || (index == 3))
   {
      config.set_grid_space(3);
      config.set_pyramid_ratio(0.5);
      config.set_number_of_pyramid_levels(4);
   }
   ...
   config.set_minimum_image_width(30);
   ...
   cv::Mat matches;
   cpm.get_matches(matches);

   EpicFlowConfig epic_config;
   epic_config.set_verbose(1);
   EpicFlowInterface epic;
   cv::Mat first_frame, second_frame;
   f1.convertTo(first_frame, CV_32F);
   f2.convertTo(second_frame, CV_32F);
   epic.run_epic_flow(first_frame, second_frame, matches, epic_config);
   ...
   cv::imwrite("estimated-epic.png", flow_img);
   of.save_estimated_flow_to_file("estimated-epic.flo");
   ...
   if (gt_name.size())
   {
      ...
      //std::cout << "Estimation with post-processing is much better!\n";
   }
