...
#ifdef __APPLE__
    // On Mac, the address returned by malloc has already been aligned
...
static void convolve_vert_fast_3(image_t *dst, const image_t *src, const convolution_t *conv){
    ...
    for(i=iterline ; --i ; ){ // first line
        ...
        float coeff2 = coeff[2];
        v4sf c01 = {coeff01, coeff01, coeff01, coeff01};
        v4sf c2 = {coeff2, coeff2, coeff2, coeff2};
    ...
    for(i=src->height-1 ; --i ; ){ // others line
        ...
        for(j=iterline ; --j ; ){
#ifdef __APPLE__
            v4sf c0 = {coeff[0], coeff[0], coeff[0], coeff[0]};
            v4sf c1 = {coeff[1], coeff[1], coeff[1], coeff[1]};
            v4sf c2 = {coeff[2], coeff[2], coeff[2], coeff[2]};
            *dstp = c0*(*srcp_m1) + c1*(*srcp) + c2*(*srcp_p1);
#else
...
#ifdef __APPLE__
        v4sf c0 = {coeff[0], coeff[0], coeff[0], coeff[0]};
        float coeff12 = coeff[1] + coeff[2];
        v4sf c12 = {coeff12, coeff12, coeff12, coeff12};
        *dstp = c0*(*srcp_m1) + c12*(*srcp);
#else
...
static void convolve_vert_fast_5(image_t *dst, const image_t *src, const convolution_t *conv){
    ...
    for(i=iterline ; --i ; ){ // first line
#ifdef __APPLE__
        float coeff012 = coeff[0]+coeff[1]+coeff[2];
        v4sf c012 = {coeff012, coeff012, coeff012, coeff012};
        v4sf c3 = {coeff[3], coeff[3], coeff[3], coeff[3]};
        v4sf c4 = {coeff[4], coeff[4], coeff[4], coeff[4]};
        *dstp = c012*(*srcp) + c3*(*srcp_p1) + c4*(*srcp_p2);
#else
    ...
    for(i=iterline ; --i ; ){ // second line
        ...
        v4sf c3 = {coeff[3], coeff[3], coeff[3], coeff[3]};
        v4sf c4 = {coeff[4], coeff[4], coeff[4], coeff[4]};
        *dstp = c01*(*srcp_m1) + c2*(*srcp) + c3*(*srcp_p1) + c4*(*srcp_p2);
#else
    ...
    for(i=src->height-3 ; --i ; ){ // others line
        ...
        for(j=iterline ; --j ; ){
            ...
            v4sf c3 = {coeff[3], coeff[3], coeff[3], coeff[3]};
            v4sf c4 = {coeff[4], coeff[4], coeff[4], coeff[4]};
            *dstp = c0*(*srcp_m2) + c1*(*srcp_m1) + c2*(*srcp) + c3*(*srcp_p1) + c4*(*srcp_p2);
#else
    ...
    for(i=iterline ; --i ; ){ // second to last line
        ...
        v4sf c1 = {coeff[1], coeff[1], coeff[1], coeff[1]};
        v4sf c2 = {coeff[2], coeff[2], coeff[2], coeff[2]};
        float coeff34 = coeff[3]+coeff[4];
        v4sf c34 = {coeff34, coeff34, coeff34, coeff34};
        *dstp = c0*(*srcp_m2) + c1*(*srcp_m1) + c2*(*srcp) + c34*(*srcp_p1);
#else
    ...
    for(i=iterline ; --i ; ){ // last line
        ...
        v4sf c0 = {coeff[0], coeff[0], coeff[0], coeff[0]};
        v4sf c1 = {coeff[1], coeff[1], coeff[1], coeff[1]};
        float coeff234 = coeff[2] + coeff[3]+coeff[4];
        v4sf c234 = {coeff234, coeff234, coeff234, coeff234};
        *dstp = c0*(*srcp_m2) + c1*(*srcp_m1) + c234*(*srcp);
#else
