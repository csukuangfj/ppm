...
struct Tint_float_t {
  ...
  float f;
  Tint_float_t(){}
  Tint_float_t(Ti i_, float f_):i(i_),f(f_){}
};
...
static int cmp_long_float ( const void* a, const void* b ) {
  long diff = ((long_float_t*)const_cast<void*>(a))->i - ((long_float_t*)const_cast<void*>(b))->i;
    ...
    const int nn = nnf->tx;
    {
        ...
        for(int i = 0 ; i<nnf->ty ; i++){
            ...
            AFFINE_ADDPOINT(m,v,xi,yi-0.1f,ui,vi,coefi);
            {
               ...
               float work_sz;
               char tmp_str[] = "Transposed";
               sgels_(tmp_str, &mm, &n, &nrhs, mat, &lda,
